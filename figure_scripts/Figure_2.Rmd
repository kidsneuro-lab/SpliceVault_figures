...
# Figure 2A: % of mis-splicing events seen in 300K-RNA at different event ranks
...
# Figure 2B: PR curve of our 88 variants, SpliceAI & 300K-RNA predictions of exon-skipping & cryptic-activation
...
B <- ggplot(plot_df[!is.na(sai_threshold)], aes(x = sens, y = ppv, color = alg)) +
...
# Figure 2C: TP / FP dot plots for exon skipping

#spliceai & MMSplice delta jitter plot
...
C <- ggarrange(es_sv, es_sai, nrow = 1)  %>% annotate_figure(fig.lab = 'Exon Skipping', fig.lab.size = 12)
```

# Figure 2D: TP / FP dot plots for cryptic activation
...
D <- ggarrange(ca_sv, ca_sai, nrow = 1) %>% annotate_figure(fig.lab = 'Cryptic Activation', fig.lab.size = 12)
...
```{r}
top <- ggarrange(A,B, nrow  = 1, widths = c(1, 1.5))
bottom <- ggarrange(C,D,nrow = 1)
ggarrange(top,bottom, ncol = 1, heights = c(1, 1.2))
ggsave('../figs/figure_two.pdf', height = 7, width = 9 , device=cairo_pdf)
``` 
